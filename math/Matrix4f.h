...
namespace math
{
	...
	public:
		...
		inline Matrix4f() { makeIdentity(); };
		inline Matrix4f( const Matrix4f& mat) { set(mat.ptr()); };
		Matrix4f(value_type a00, value_type a01, value_type a02,value_type a03,
		         value_type a10, value_type a11, value_type a12,value_type a13,
		         value_type a20, value_type a21, value_type a22,value_type a23,
		         value_type a30, value_type a31, value_type a32,value_type a33);
		inline explicit Matrix4f( value_type const * const ptr ) { set(ptr); };
		inline explicit Matrix4f( double const * const ptr ) { set(ptr); };
		...
		value_type* ptr() { return (value_type*)_mat; };
            ...
            // basic Matrix4f multiplication, our workhorse methods.
            void mult( const Matrix4f&, const Matrix4f& );
            void preMult( const Matrix4f& );
            void postMult( const Matrix4f& );
            ...
            inline void operator *= ( const Matrix4f& other ) 
            {    if( this == &other ) {
                                          Matrix4f temp(other);
                                          postMult( temp );
                                      }
            ...
            int compare(const Matrix4f& m) const;
            bool operator < (const Matrix4f& m) const { return compare(m)<0; }
            bool operator == (const Matrix4f& m) const { return compare(m)==0; }
            bool operator != (const Matrix4f& m) const { return compare(m)!=0; }

        inline void set(const Matrix4f& rhs) { set(rhs.ptr()); };
        ...
        inline Vec3f getTrans() const { return Vec3d(_mat[3][0],_mat[3][1],_mat[3][2]); } 
        ...
        inline Vec3f getScale() const {
          Vec3f x_vec(_mat[0][0],_mat[1][0],_mat[2][0]); 
          Vec3f y_vec(_mat[0][1],_mat[1][1],_mat[2][1]); 
          Vec3f z_vec(_mat[0][2],_mat[1][2],_mat[2][2]); 
          return Vec3f(x_vec.length(), y_vec.length(), z_vec.length()); 
        }
	protected:
		value_type	_mat[4][4];
...
inline Vec3f Matrix4f::postMult( const Vec3f& v ) const
{
...
inline Vec3d Matrix4f::postMult( const Vec3d& v ) const
{
    value_type d = 1.0f/(_mat[3][0]*v.x()+_mat[3][1]*v.y()+_mat[3][2]*v.z()+_mat[3][3]) ;
...
inline Vec3f Matrix4f::preMult( const Vec3f& v ) const
{
...
inline Vec3d Matrix4f::preMult( const Vec3d& v ) const
{
    value_type d = 1.0f/(_mat[0][3]*v.x()+_mat[1][3]*v.y()+_mat[2][3]*v.z()+_mat[3][3]) ;
...
inline Vec4f Matrix4f::postMult( const Vec4f& v ) const
{
...
inline Vec4d Matrix4f::postMult( const Vec4d& v ) const
{
...
inline Vec4f Matrix4f::preMult( const Vec4f& v ) const
{
...
inline Vec4d Matrix4f::preMult( const Vec4d& v ) const
{
...
inline Vec3f operator* (const Vec3f& v, const Matrix4f& m )
{
    return m.preMult(v);
}

inline Vec3d operator* (const Vec3d& v, const Matrix4f& m )
{
    return m.preMult(v);
}

inline Vec4f operator* (const Vec4f& v, const Matrix4f& m )
{
    return m.preMult(v);
}

inline Vec4d operator* (const Vec4d& v, const Matrix4f& m )
{
    return m.preMult(v);
}

inline Vec3f Matrix4f::operator* (const Vec3f& v) const
{
    return postMult(v);
}

inline Vec3d Matrix4f::operator* (const Vec3d& v) const
{
    return postMult(v);
}

inline Vec4f Matrix4f::operator* (const Vec4f& v) const
{
    return postMult(v);
}

inline Vec4d Matrix4f::operator* (const Vec4d& v) const
{
    return postMult(v);
}
