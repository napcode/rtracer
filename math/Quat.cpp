...
void Quat::set(const Matrix4& matrix)
{
    *this = matrix.getRotate();
}


void Quat::get(Matrix4& matrix) const
{
...
void Quat::makeRotate_original( const Vec3d& from, const Vec3d& to )
{
    ...
    {
        //XS_NOTIFY(xs::INFO, "*** Quat::makeRotate(from,to) with near co-linear vectors, epsilon= "<<fabs(cosangle-1));
...
void test_Quat_Eueler(value_type heading,value_type pitch,value_type roll)
{
    ...
    xs::Matrix4 r_m = osg::Matrix::rotate(roll,0.0,1.0,0.0)*
